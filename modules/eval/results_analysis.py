...
def plot_quick_comparison(
    ...
    if len(model_names) != 2:
        raise ValueError(
            "This function currently supports exactly two models for comparison."
        )
    ...
    plt.tight_layout()
    plt.suptitle(
        f"Comparison of ROUGE Score Distributions: {model_1} vs {model_2}",
        fontsize=16,
        fontweight="bold",
        y=1.02,
    )
...
def plot_detailed_comparison(
    ...
    fig, axes = plt.subplots(
        nrows=len(score_columns), ncols=2, figsize=(14, 4 * len(score_columns))
    )
    ...
    plt.tight_layout()
    plt.suptitle(
        f"Comparison of ROUGE Score Histograms: {model_1} vs {model_2}",
        fontsize=16,
        fontweight="bold",
        y=1.02,
    )
...
def get_results_for_zero_scores(
    data: pd.DataFrame, model_name: str, output_path: Path, save: bool = False
) -> pd.DataFrame:
    """
...
def get_outperformed_results(
    ...
    better_results = merged_df[
        ...
        & (
            merged_df[f"rougeLsum_{first_model}"]
            > merged_df[f"rougeLsum_{second_model}"]
        )
    ]
...
def calculate_averages(data: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    averages = (
        data.groupby("generated_size")[["rouge1", "rouge2", "rougeL", "rougeLsum"]]
        .mean()
        .reset_index()
    )
    averages.columns = [
...
def plot_by_group_sizes(df: pd.DataFrame, output_path: Path) -> None:
    """
    ...
    for i, score in enumerate(score_columns):
        ...
        axes[i].plot(
            ranked_sizes["size"].astype(str),
            ranked_sizes["score_diff"],
            label="Score Difference",
            color=line_color,
            marker="o",
        )
        axes[i].set_title(
            f"{score.replace('_', ' ').upper()} Rankings and Score Differences"
        )
...
def compute_cosine_similarities(
    df: pd.DataFrame, model_ST: SentenceTransformer
) -> List[float]:
    """
    ...
    cosine_similarities = []
    for _, row in tqdm(
        df.iterrows(), total=len(df), desc="Computing Cosine Similarities"
    ):
...
def plot_similarity_vs_rouge(filtered_df: pd.DataFrame, output_path: Path) -> None:
    """
...
def plot_semantic_similarity_distribution(
    filtered_df: pd.DataFrame, output_path: Path
) -> None:
    """
    ...
    plt.savefig(
        f"{output_path}/semantic_similarity_distribution.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def plot_average_scores_by_model(
    data: pd.DataFrame, score_columns: List[str], output_path: Path
) -> None:
    """
    ...
    plt.tight_layout()
    plt.suptitle(
        "Average ROUGE Scores by Model", fontsize=16, fontweight="bold", y=1.02
    )

    # Save and display the plot
    plt.savefig(
        f"{output_path}/average_scores_by_model.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def main():
    ...
    input_path = Path(config["results_path"])
    ...
    logger.info(
        f"Loaded dataset with { results.shape[0]} rows and {results.shape[1]} columns"
    )
    ...
    if len(models) > 2:
        logger.warning(
            "There are more than two models. Using two models set in config."
        )
        first_model, second_model = (
            models[config["compare_models"][0]],
            models[config["compare_models"][1]],
        )
    ...
    plot_detailed_comparison(
        results, score_columns, [first_model, second_model], output_path
    )
    plot_quick_comparison(
        results, score_columns, [first_model, second_model], output_path
    )
    plot_average_scores_by_model(results, score_columns, output_path)

    # Retrieve and print various results
    get_outperformed_results(
        results,
        second_model,
        first_model,
        save=config["save_outperformed"],
        output_path=output_path,
    )
    get_results_for_zero_scores(
        results, first_model, save=config["save_zeros"], output_path=output_path
    )
    get_best_results(
        data=results,
        score=50,
    ...
    fine_tuned_df["generated_size"] = fine_tuned_df["generated_output"].apply(
        lambda x: len(x.split())
    )
    averages = calculate_averages(
        fine_tuned_df[["generated_size", "rouge1", "rouge2", "rougeL", "rougeLsum"]]
    )
    logger.info("Averages by generated output size:")
    logger.info(averages)
    plot_by_group_sizes(averages, output_path)
    ...
    if config["compute_similarity"]:
        ...
        plot_similarity_vs_rouge(filtered_df, output_path)
        plot_semantic_similarity_distribution(filtered_df, output_path)
